...
class Dataset:
	...
	def load(self, img_rows = 56, img_cols = 116, 
		...
		cutImage2 = np.array(images)	
		cutImage3 = np.array(images)			
		cutImage2 = cutImage2.reshape(cutImage2.shape[0], img_rows, img_cols, img_channels)
		cutImage3 = cutImage3.reshape(cutImage3.shape[0], img_rows, img_cols, img_channels)
...
class makeModel:
	...
	def build_model(self, nb_classes = 2):
		...
		self.model = Model(inputs = [main_inputI3,inputI2,inputI1,box],outputs = main_output)
		self.model.summary()
	
	#訓練模型函數
	def train(self, dataset, batch_size = 8, epochs = 50):        
		#優化器參數設定
		adam = Adam(lr=0.0001, beta_1=0.9, beta_2=0.999, decay=0.000001, amsgrad=False)
		...
		for train, test in kfold.split(dataset.train_images,dataset.train_labels):#train, test為訓練圖片與測試團片的index list 
			...
			#執行模型
			self.model.fit([dataset.train_images[train],dataset.cutImage2[train],dataset.cutImage3[train],dataset.train_box[train]],dataset.train_labels[train],batch_size = batch_size,epochs = epochs,shuffle = True)
			#衡量模型
			loss, accuracy = self.model.evaluate([dataset.train_images[test],dataset.cutImage2[test],dataset.cutImage3[test],dataset.train_box[test]], dataset.train_labels[test], verbose=1)
			...
			y_preds = self.model.predict([dataset.train_images[test],dataset.cutImage2[test],dataset.cutImage3[test],dataset.train_box[test]])
			#y_preds為預測的結果
...
#將圖片切三等分的方法中，此方法為儲存中間(中期)圖片，把指定部分變成0
...
#將圖片切三等分的方法中，此方法為儲存短期圖片，把指定部分變成0
...
if __name__ == '__main__':
	...
	print("圖片")
	#儲存模型
	model.save_model(file_path = 'stock_data.h5')
